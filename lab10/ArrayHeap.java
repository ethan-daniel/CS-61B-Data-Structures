...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        ...
        while (2 * index <= size) {
            ...
            if (min(left, size) == left && min(left, left + 1) == left + 1) {
                ++left;
            }
            if (min(index, left) == index) {
                break;
            }
            swap(index, left);
            index = left;
    ...
    public void changePriority(T item, double priority) {
        ...
        if (contents[index].priority() < contents[leftChild].priority()
                || contents[index].priority() < contents[rightChild].priority()) {
            sink(index);
        }
    }
